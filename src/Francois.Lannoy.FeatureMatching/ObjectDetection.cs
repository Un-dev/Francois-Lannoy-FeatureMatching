...
namespace Francois.Lannoy.FeatureMatching
{
    public class ObjectDetection
    {
        public IList<ObjectDetectionResult> DetectObjectInScenes(byte[] objectImageData, IList<byte[]> imageSceneData)
        {
            IList<ObjectDetectionResult> res = new List<ObjectDetectionResult>();
            
            /*foreach(byte[] data in imageSceneData)
            {
                res.Add(DetectObjectInScene(objectImageData, data));
            }*/
        ...
        public ObjectDetectionResult DetectObjectInScene(byte[] imageObjectData, byte[] imageSceneData)
        {
            ...
            return new ObjectDetectionResult()
            {
                ...
                /*
                Points = {
                    new ObjectDetectionPoint(){X=116, Y=158},
